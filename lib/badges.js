...
//
// There is only one badge theme for everyone - the pink footer band sampled
// directly from public/images/"visitor badge.jpeg" - no per-tier colors.
// Only the id prefix and the footer's role label differ by tier.
export const BADGE_THEME_COLOR = "#ed0090";

export const BADGE_TIERS = {
  platinum: { prefix: "PLT", label: "PLATINUM DELEGATE" },
  gold: { prefix: "GLD", label: "GOLD DELEGATE" },
  silver: { prefix: "SLV", label: "SILVER DELEGATE" },
  visitor: { prefix: "VIS", label: "VISITOR" },
};
...
export async function generateBadge({ tierKey, numericId, name, company, roleLabel, siteOrigin }) {
  ...
  const overlaySvg = `<svg xmlns="http://www.w3.org/2000/svg" width="${TEMPLATE_WIDTH}" height="${TEMPLATE_HEIGHT}" viewBox="0 0 ${TEMPLATE_WIDTH} ${TEMPLATE_HEIGHT}">
    ...
    <rect x="0" y="${FOOTER_TOP}" width="${TEMPLATE_WIDTH}" height="${FOOTER_HEIGHT}" fill="${BADGE_THEME_COLOR}"/>
